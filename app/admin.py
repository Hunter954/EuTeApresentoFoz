...
from .models import db, User, AdSlot, SiteSetting, PageView, Post, Category, post_categories, AnalyticsSession, Product, CityPhoto, BeforeAfter, WhatsAppGroup, ChatMessage
...
def insights_page():
    ...
    return render_template(
        ...
        contact_address=_setting("contact_address", ""),
        weather_text=_setting("weather_text", ""),
...
def _creative_config(kind: str):
    configs = {
        'products': {
            'model': Product,
            'section': 'products',
            'title': 'Lojinha Nostálgica',
            'singular': 'produto',
            'subtitle': 'Cadastre produtos nostálgicos para aparecerem na home e na página da lojinha.',
            'public_endpoint': 'site.products_page',
            'display_attr': 'name',
            'slug_attr': 'slug',
            'image_fields': [{'name': 'image_url', 'label': 'Imagem do produto'}],
            'fields': [
                {'name': 'name', 'label': 'Nome', 'type': 'text', 'required': True},
                {'name': 'slug', 'label': 'Slug', 'type': 'text'},
                {'name': 'price_label', 'label': 'Preço / etiqueta', 'type': 'text', 'placeholder': 'R$ 34,90'},
                {'name': 'buy_url', 'label': 'Link de compra', 'type': 'url', 'placeholder': 'https://...'},
        ...
        'photos': {
            ...
            'singular': 'foto',
            'subtitle': 'Galeria de fotos e imagens da cidade exibida na home e na página de fotos.',
            'public_endpoint': 'site.city_photos_page',
            'display_attr': 'title',
            'image_fields': [{'name': 'image_url', 'label': 'Imagem'}],
            'fields': [
        ...
        'before-after': {
            ...
            'singular': 'comparação',
            'subtitle': 'Cadastre pares de imagens antigas e atuais para a sessão Antes e Depois.',
            'public_endpoint': 'site.before_after_page',
            'display_attr': 'title',
            'image_fields': [
                {'name': 'before_image', 'label': 'Imagem antiga / antes'},
                {'name': 'after_image', 'label': 'Imagem atual / depois'},
            ],
            'fields': [
                {'name': 'title', 'label': 'Título', 'type': 'text', 'required': True},
                {'name': 'location', 'label': 'Local', 'type': 'text'},
                {'name': 'year_before', 'label': 'Ano / texto do antes', 'type': 'text', 'placeholder': '1988'},
                {'name': 'year_after', 'label': 'Ano / texto do depois', 'type': 'text', 'placeholder': 'Hoje'},
                {'name': 'sort_order', 'label': 'Ordem', 'type': 'number'},
            ],
        },
        'whatsapp-groups': {
            'model': WhatsAppGroup,
            'section': 'whatsapp_groups',
            'title': 'Grupos WhatsApp',
            'singular': 'grupo',
            'subtitle': 'Cadastre grupos da cidade com link de convite, descrição e quantidade de membros.',
            'public_endpoint': 'site.whatsapp_groups_page',
            'display_attr': 'name',
            'image_fields': [],
            'fields': [
                {'name': 'name', 'label': 'Nome', 'type': 'text', 'required': True},
                {'name': 'description', 'label': 'Descrição', 'type': 'textarea'},
                {'name': 'member_count', 'label': 'Membros', 'type': 'text', 'placeholder': '1.842 membros'},
                {'name': 'link_url', 'label': 'Link do grupo', 'type': 'url', 'placeholder': 'https://chat.whatsapp.com/...'},
                {'name': 'icon_class', 'label': 'Ícone Bootstrap', 'type': 'text', 'placeholder': 'bi-people-fill'},
                {'name': 'sort_order', 'label': 'Ordem', 'type': 'number'},
            ],
        },
    }
...
def _creative_ordered_query(model):
    return model.query.order_by(model.sort_order.asc(), model.created_at.desc())


def _bind_creative_item(item, cfg):
    old_images = {img['name']: (getattr(item, img['name'], '') or '') for img in cfg.get('image_fields', [])}
    for field in cfg.get('fields', []):
        ...
        if field.get('type') == 'number':
            try:
                setattr(item, name, int(raw or 0))
            except Exception:
                setattr(item, name, 0)
        else:
            setattr(item, name, (raw or '').strip())

    if cfg.get('slug_attr'):
        desired_slug = (request.form.get('slug') or getattr(item, cfg['display_attr'], '') or cfg['singular']).strip()
        setattr(item, cfg['slug_attr'], _ensure_unique_slug(cfg['model'], desired_slug, object_id=getattr(item, 'id', None)))

    for img in cfg.get('image_fields', []):
        name = img['name']
        current_value = (request.form.get(name) or '').strip()
        upload = request.files.get(f'{name}_file')
        if upload and getattr(upload, 'filename', ''):
            current_value = _save_upload(upload, cfg['section'])
        setattr(item, name, current_value)
...
@admin_bp.get('/conteudo/<kind>')
@login_required
def creative_list(kind):
    r = _require_admin()
    if r:
        return r
    cfg = _creative_config(kind)
    if not cfg:
        abort(404)
    items = _creative_ordered_query(cfg['model']).all()
    return render_template('admin/creative_list.html', cfg=cfg, kind=kind, items=items, **_common_admin_context('creative'))


@admin_bp.route('/conteudo/<kind>/novo', methods=['GET', 'POST'])
@login_required
def creative_new(kind):
    r = _require_admin()
    if r:
        return r
    cfg = _creative_config(kind)
    ...
    if request.method == 'POST':
        _bind_creative_item(item, cfg)
        display = (getattr(item, cfg['display_attr'], '') or '').strip()
        if not display:
            flash(f'Informe o nome/título do {cfg["singular"]}.', 'danger')
        else:
            if cfg.get('slug_attr') and not getattr(item, cfg['slug_attr'], ''):
                setattr(item, cfg['slug_attr'], _ensure_unique_slug(cfg['model'], display))
            db.session.add(item)
            db.session.commit()
            flash(f'{cfg["singular"].capitalize()} cadastrado com sucesso.', 'success')
            return redirect(url_for('admin.creative_list', kind=kind))
    return render_template('admin/creative_form.html', cfg=cfg, kind=kind, item=item, mode='new', **_common_admin_context('creative'))


@admin_bp.route('/conteudo/<kind>/<int:item_id>/editar', methods=['GET', 'POST'])
@login_required
def creative_edit(kind, item_id):
    r = _require_admin()
    if r:
        return r
    cfg = _creative_config(kind)
    ...
    if request.method == 'POST':
        old_images = _bind_creative_item(item, cfg)
        display = (getattr(item, cfg['display_attr'], '') or '').strip()
        if not display:
            flash(f'Informe o nome/título do {cfg["singular"]}.', 'danger')
        else:
            db.session.commit()
            for name, old_url in old_images.items():
                new_url = getattr(item, name, '') or ''
                if old_url and old_url != new_url:
                    _delete_local_media(old_url)
            flash(f'{cfg["singular"].capitalize()} atualizado com sucesso.', 'success')
            return redirect(url_for('admin.creative_list', kind=kind))
    return render_template('admin/creative_form.html', cfg=cfg, kind=kind, item=item, mode='edit', **_common_admin_context('creative'))


@admin_bp.post('/conteudo/<kind>/<int:item_id>/excluir')
@login_required
def creative_delete(kind, item_id):
    r = _require_admin()
    if r:
        return r
    cfg = _creative_config(kind)
    if not cfg:
        abort(404)
    item = cfg['model'].query.get_or_404(item_id)
    image_urls = [getattr(item, img['name'], '') or '' for img in cfg.get('image_fields', [])]
    db.session.delete(item)
    db.session.commit()
    for url in image_urls:
        _delete_local_media(url)
    flash(f'{cfg["singular"].capitalize()} excluído com sucesso.', 'success')
    return redirect(url_for('admin.creative_list', kind=kind))


@admin_bp.get('/conteudo/chat')
@login_required
def chat_messages():
    r = _require_admin()
    if r:
        return r
    messages = ChatMessage.query.order_by(desc(ChatMessage.created_at)).limit(300).all()
    return render_template('admin/chat_messages.html', messages=messages, **_common_admin_context('creative'))


@admin_bp.post('/conteudo/chat/<int:message_id>/toggle')
@login_required
def chat_message_toggle(message_id):
    r = _require_admin()
    if r:
        return r
    ...
    return redirect(url_for('admin.chat_messages'))


@admin_bp.post('/conteudo/chat/<int:message_id>/delete')
@login_required
...
def save_footer_social():
    ...
    _save_setting("footer_copyright_text", (request.form.get("footer_copyright_text", "") or "").strip() or "© 2026 Eu Te Apresento Foz. Todos os direitos reservados.")
...
def save_logo():
    ...
    _save_setting("contact_address", (request.form.get("contact_address", "") or "").strip())
    _save_setting("weather_text", (request.form.get("weather_text", "") or "").strip())
    _save_setting("announcement_url", (request.form.get("announcement_url", "") or "").strip())
    _save_setting("instagram_url", (request.form.get("instagram_url", "") or "").strip())
    _save_setting("facebook_url", (request.form.get("facebook_url", "") or "").strip())
    _save_setting("youtube_url", (request.form.get("youtube_url", "") or "").strip())
    _save_setting("x_url", (request.form.get("x_url", "") or "").strip())
